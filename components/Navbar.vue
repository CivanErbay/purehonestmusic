<template>
  ...
  >
    <DefaultGrid :no-spacing="true" class="relative flex items-center mb-4 mt-4">
      <div class="max-w-[2056px] mx-auto flex items-end w-full">
        <NuxtLink
          to="/"
      ...
      <div class="md:col-start-12 md:col-end-12 hidden md:block">
        <NuxtLink
          to="/concerts/favorites"
          class="flex w-fit ml-auto items-center p-4 bg-bg-light rounded-lg gap-4 tracking-widest transform transition-transform duration-300 hover:scale-110"
        >
          <div class="h-7 w-7 flex items-center justify-center rounded-full bg-[#E77000]">
...
const isVisible = ref(true); // Variable zur Steuerung der Sichtbarkeit der Navbar
let lastScroll = 0;

// Funktion zur Handhabung des Scrollens
const handleScroll = () => {
  const currentScroll = window.scrollY;

  // Wenn nach unten gescrollt wird und die Navbar noch sichtbar ist, ausblenden
  if (currentScroll > lastScroll && currentScroll > 50) {
    isVisible.value = false;
  } else {
    // Wenn nach oben gescrollt wird oder oberhalb von 50px, Navbar einblenden
    if (currentScroll < lastScroll || currentScroll <= 50) {
      isVisible.value = true;
    }
  }

  // Den letzten Scrollwert speichern
...
<style scoped>
/* Optional: Du kannst noch weitere Stil-Anpassungen hier vornehmen, falls nötig */
</style>
